...
Individuo gerar_individuo() {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        for(int j = 0; j < tamanho; j++) {
            auto tabela = obter_tabela(i, j);
            const double* limite = obter_limite(i, j);
            Porcao porcao;

            porcao.alimento = sortear_inteiro(0, tabela.size() - 1);
...
Nutrientes calcular_nutrientes(const Individuo& individuo) {
    ...
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < individuo.refeicoes[i].size(); j++) {
            Porcao porcao = individuo.refeicoes[i][j];
            Alimento alimento = obter_tabela(i, j)[porcao.alimento];
...
double avaliar_individuo(const Individuo& individuo) {
    ...
    for (int i = 1; i < 4; i += 2) {
        auto refeicao = individuo.refeicoes[i];
...
void imprimir_dieta(const Individuo& individuo) {
    ...
    for(int i = 0; i < 4; i++) {
        cout << "\n" << nomes[i] << "\n";
        int tamanho = individuo.refeicoes[i].size();
        for(int j = 0; j < tamanho; j++) {
            Porcao porcao = individuo.refeicoes[i][j];
